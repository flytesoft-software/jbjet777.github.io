...
class EclipseUI
{
    constructor()
    {
        ...
        var showRadar = $("#show-wx-radar");
        var showRadarCheck = showRadar.children("label").children("input");
        var showVisWx = $("#show-vis-sat");
        var showVisWxCheck = showVisWx.children("label").children("input");
        var showIRWx = $("#show-ir-sat");
        var showIRWCheck = showIRWx.children("label").children("input");
        ...
        function flexContentWide()
        {
            circumstances.css("flex-direction", "row");
            topID.css("flex", "1");
            bottomID.css("flex", "1");
            timeBlock2.css("top", "1.5em");
            timeBlock.addClass("eclipse-time-block-shift");
        }
        
        function unFlexContent()
        {
            circumstances.css("flex-direction", "");
            topID.css("flex", "");
            bottomID.css("flex", "");
            timeBlock2.css("top", "");
            timeBlock.removeClass("eclipse-time-block-shift");
        }
        ...
        function setPageHeights()
        {
            ...
            circumstances.css("min-height", contentHeight + "px");
            updateSimMetrics(contentHeight);
            
            if(material.getCurrentPageIndex() === CIRCUMSTANCE_PAGE_IDX)
            {
        ...
        function onMapDisplay()
        {
            ...
            mapMenuButton.show();            
            if(checkIfEclipseIsOccurring())
            {
                if(!realTimeInfoID.hasClass("eclipse-realtime-shadow-trans"))
                {
        ...
        function checkWxLayers()
        {
            if(showRadarCheck.is(':checked'))
            {
                map.wxRadarOn();
            }
            else
        ...
        function revLong(long)
        {
            if (long > 360.0 || long < 0.0)
            {
                long = (long - (Math.floor(long / 360.0) * 360.0));
            }
            if(long > 180)
            {
                long -= 360;
        ...
        function bindEvents()
        {
            ...
            showVisWxCheck.click(function()
            {
                console.log("Sat Vis click");
                var box = $(this);
                
                if(box.is(":checked"))
                {
                    console.log("Showing Sat Vis.");
                    map.wxVisSatOn();
                }
                else
                {
            ...
            showIRWCheck.click(function()
            {
                console.log("Sat IR click.");
                var box = $(this);
                
                if(box.is(":checked"))
                {
                    console.log("Showing Sat IR.");
                    map.wxIRSatOn();
                }
                else
                {
            ...
            aboutLink.click(function()
            {
                ...
                $(window).one("keydown", function(event) // Capturing 'escape' or Android back button.
                {
                    if(event.which === 27)
                    {
                       event.preventDefault();
            ...
            mapSearchInputID.focus(function(event)
            {
                ...
                if(typeof(cordova) !== 'undefined')
                {
                    $(document).one("backbutton", function(event)
                    {
                        ...
                        event.preventDefault();
                        event.stopPropagation();
                        mapSearchInputID.blur();
                        jMap.focus();
                        return false;
                    });
                }
                
                $(window).on("keydown", function(event) // Capturing 'escape' or Android back button.
                {
                    ...
                    if(event.which === 13 || event.which === 14) // Capture return or enter key.
                    {
                        $(window).off("keydown");
                        backButtonOff();
                        event.preventDefault();
                        ...
                        if(mapSearchData)
                        {
                            ...
                            {
                                var searchedPos = new PositionObj(parseFloat(mapSearchData[0].lat), parseFloat(mapSearchData[0].lon));
                                searchedPos.altitude = 1;
                                setLocationMode(false);
                                setLocation(searchedPos);
                                positionWatch.getOnlineElevation(searchedPos, function(pos)
                                {
                                    console.log("Updating position with elevation data.");
                                    setLocation(pos);                            
                                });
                                mapSearchData = null;
                            }
                        ...
                        {
                            showToast("No valid location selected.");
                        }
            ...
            mapSearchList.click(function(event)
            {
                ...
                if(mapSearchData)
                {
                    ...
                    if(idx < mapSearchData.length)
                    {
                        ...
                        var searchedPos = new PositionObj(parseFloat(mapSearchData[idx].lat), parseFloat(mapSearchData[idx].lon));
                        searchedPos.altitude = 1;
                        setLocationMode(false);
                        setLocation(searchedPos);
                        positionWatch.getOnlineElevation(searchedPos, function(pos)
        ...
        {
            var bEclipseOccurs = false;
            
            if(currentEclipseRef)
            {
                var current_date = new Date();
                current_date.setTime(current_date.getTime() + dateOffset);
                bEclipseOccurs = currentEclipseRef.isEclipseOccurring(current_date);
                if (bEclipseOccurs && !shadowAnimator.isAnimating())
                {
        ...
        {
            currentCoords.longitude = revLong(coords.longitude);
            currentCoords.latitude = revLat(coords.latitude);
            ...
            latID.html(currentCoords.latitude.toFixed(2));
            longID.html(currentCoords.longitude.toFixed(2));
            altID.html((currentAltitude * METERS_TO_FEET).toFixed(0));
        ...
        function setLocationMode(bMode)
        {
            if(bMode)
            {
                if(!positionWatch.isOn())
                {
                    ...
                    showToast("Aquiring GPS position...");
                    console.log("Aquiring GPS position...");
                    
                    map.clickOff();
                    positionWatch.onFirstPostion(function(coords)
                    {
                        ...
                        if(!coords.altitude)
                        {
                            positionWatch.getOnlineElevation(coords, function(coords)
                            {
                                setLocation(coords);
                            });
                        }
            ...
            else
            {
                ...
                map.onClick(function(position)
                {
                    ...
                    if(!longCickFired)
                    {
                        ...
                        positionWatch.getOnlineElevation(currentCoords, function(pos)
                        {
                            console.log("Updating position with elevation data.");
                            setLocation(pos);                            
                        });
                    ...
                    {
                        console.log("Map ignoring long click.");
                        longCickFired = false;
                    }
                });
                ...
                console.log("Position in manual mode.");
            }
        }
...
class WatchPosition
{
    getOnlineElevation(position, callback)
    {
        if(typeof(position.latitude) === "number" && typeof(position.longitude) === "number" && typeof(callback) === "function")
        {
            var inputStr = "https://nationalmap.gov/epqs/pqs.php?x=LONG&y=LAT&units=Meters&output=json";
            
            inputStr = inputStr.replace("LONG", position.longitude.toString());
            inputStr = inputStr.replace("LAT", position.latitude.toString());
            console.log("Getting elevation data.");
            
            $.getJSON(inputStr, function() 
            {
                console.log("Initial elevation success.");
            }).done(function (data) 
            {
                console.log("Elevation callback success.");
                if(typeof(data.USGS_Elevation_Point_Query_Service.Elevation_Query.Elevation) === 'number' ||
                       typeof(data.USGS_Elevation_Point_Query_Service.Elevation_Query.Elevation) === 'string' )
                {
                    var alt = parseFloat(data.USGS_Elevation_Point_Query_Service.Elevation_Query.Elevation);

                    if(alt > -999)
                    {
                        callback(new PositionObj(position.latitude, position.longitude, alt));
                    }
                    else
                    {
                        console.log("Bad elevation number, possibly non covered area or oceanic.");
                    }
                }
                else
                {
                    console.log("Elevation returned malformed data.");
                }
            }).fail(function () 
            {
        ...
        {
            throw new Error("Invalid input to getOnlineElevation function.");
        }
    }
